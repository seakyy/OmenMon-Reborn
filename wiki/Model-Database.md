...
| `8A3E` | Victus 15-fb0102la (2023) | ✅ Read-only RPM mapping at `0xB0`/`0xB2` (16-bit LE; CPU idle ≈435 — this fan never fully stops — max ≈5404; GPU 0…≈5195 RPM), confirmed against the issue #96 Auto-Calibration sweep (@David112x). Fan control uses the default 2022 layout, which the sweep drove successfully (level `0x34`/`0x35` tracked every step). |
| `88ED` | Victus 16-e0xxx (2022) | ✅ Native entry since v1.4.6 (issue #99, @robbert1978): classic 2022 layout (level `0x34`/`0x35`, rate write `0x2C`/`0x2D`), RPM at `0xB0`/`0xB2` (16-bit LE; CPU max ≈3138, GPU ≈3369). Field-tested by the reporter; identical to what the auto-detector already chose, now pinned. Note: the 100 % step gains <150 RPM over 70 % on this board — the level scale reaches the physical ceiling early, this is normal. |
| `8D87` | Omen (2025, ak0xxx) | ✅ Native entry (issue #61): 2023+ layout, RPM at `0x70`/`0x9F` (16-bit LE). **Temperature quirk (issue #97):** legacy CPUT `0x57` lands on firmware string data and reads a constant 52; since v1.4.6 the CPU temperature takes the higher of CPUT and the WMI BIOS sensor, so fan curves follow the real temperature. A proper `TempCpuReg` override still needs a `-Probe` dump under load. |
| `8787` | Omen 15-en0037AX (2020) | ✅ Default 2022 layout confirmed via the issue #95 hardware probe (@sawankumar): fan levels at `0x34`/`0x35`, RPM at `0xB0`/`0xB2` (16-bit LE, CPU ≈2400 RPM at idle), BIOS `GetTemperature` works (39 °C). The probe's `GetFanLevel` → "Unknown response from BIOS: 45" is expected on this 2020 generation — the WMI fan-level call is unsupported there and OmenMon reads levels from the EC instead; no functional impact. |
...
| HP Victus 16-s1084AX | #100 | `-Diag` Product ID confirmation. The issue title says `898C` but the pasted Auto-Calibration report says `8C9C` — and `8C9C` already ships a hand-verified, *different* mapping (BIOS-level mirror + temp remap, Victus 16-1034NF). The s1084AX sweep found real 16-bit LE tachometers at `0xD6`/`0xD8` (CPU/GPU, max ≈6051/6020 RPM), which would be a second hardware variant behind the same ID — changing the shipped `8C9C` mapping on one report risks regressing 1034NF owners (HP recycles product IDs across regional variants, see `8C77`). The reporter's own wizard sidecar (`OmenMon-AutoCal.xml`) already applies the `0xD6`/`0xD8` mapping locally, so their install works today. Needed: the `-Diag` "Model Database Match" section from the s1084AX, plus ideally one more report from either SKU. |
