...
#region Visual Methods
        ...
        public void Update() {
            ...
            if(this.UpdateIconTick++ == 0) {
                ...
                if(this.Icon.IsDynamic) {
                    ...
                    // The dynamic icon uses the custom resource font: only the localized °C glyph
                    // is guaranteed to render; passing a literal "°F" may produce garbled output.
                    // When Fahrenheit is active, omit the unit suffix from the icon (the tooltip
                    // still shows the full unit label).
        ...
        private string BuildTrayTooltip() {
            try {
                ...
                // Fan RPMs are visible in the main form; omit them from the tooltip to avoid
                // calling GetSpeed() (WinRing0 EC read) a second time per tick.
